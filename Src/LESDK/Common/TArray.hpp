...
public:  // Standard container compatibility.
    ...
    AsVec(Allocator const& Alloc = Allocator()) const noexcept;
...
/**
 * @brief   Dynamic array managed by Unreal Engine.
 * @tparam  T Element type.
 */
template<TArrayElement T>
class TArray final : public TArrayBase<T, false> {
    CONTAINER_TYPEDEFS(T, UINT, INT)
...
public:
    ...
    TArray(TArray const& Other) = default;
    TArray& operator=(TArray const& Other) = default;
    TArray(TArray&& Other) = delete;
    TArray& operator=(TArray&& Other) = delete;
};
...
/**
 * @brief   Dynamic array managed by plugin-side code.
 * @tparam  T Element type.
 * @remarks Essentially it is a @ref TArray with RAII support.
 */
template<TArrayElement T>
class TArrayRAII final : public TArrayBase<T, true> {
    CONTAINER_TYPEDEFS(T, UINT, INT)

public:
    TArrayRAII() : TArrayBase{} {}
    explicit TArrayRAII(size_type Capacity);
    TArrayRAII(size_type Count, const_reference Value);
    TArrayRAII(std::initializer_list<value_type> const List) : TArrayBase{ List } {}

    ~TArrayRAII() noexcept {}

    // Unlike with TArray, copy / move semantics here actually copy / destroy contents.

    TArrayRAII(TArrayRAII const& Other);
    TArrayRAII& operator=(TArrayRAII const& Other);
    TArrayRAII(TArrayRAII&& Other) noexcept;
    TArrayRAII& operator=(TArrayRAII&& Other) noexcept;

};

static_assert(std::is_copy_assignable_v<TArrayRAII<int>>);
static_assert(std::is_copy_constructible_v<TArrayRAII<int>>);
static_assert(std::is_move_assignable_v<TArrayRAII<int>>);
static_assert(std::is_move_constructible_v<TArrayRAII<int>>);

template<TArrayElement T>
TArrayRAII<T>::TArrayRAII(size_type const Capacity)
    : TArrayBase{}
{
...
template<TArrayElement T>
TArrayRAII<T>::TArrayRAII(size_type const Count, const_reference Value)
    : TArrayBase{}
{
...
template<TArrayElement T>
TArrayRAII<T>::TArrayRAII(TArrayRAII const& Other)
    : TArrayRAII{}
{
...
template<TArrayElement T>
TArrayRAII<T>& TArrayRAII<T>::operator=(TArrayRAII const& Other) {
    if (this != &Other) {
...
template<TArrayElement T>
TArrayRAII<T>::TArrayRAII(TArrayRAII&& Other) noexcept
    : TArrayRAII{}
{
...
template<TArrayElement T>
TArrayRAII<T>& TArrayRAII<T>::operator=(TArrayRAII&& Other) noexcept {
    if (this != &Other) {
        this->DoDestroyContents();
        this->Data = std::exchange(Other.Data, nullptr);
        this->CountItems = std::exchange(Other.CountItems, 0);
